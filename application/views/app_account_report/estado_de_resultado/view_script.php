				...
				<script>				
					$(document).ready(function(){
						$(document).on("click","#print-btn-report",function(){
							...
							if(!(periodID == "" || cycleID == "") ){
								fnWaitOpen();
								window.location	= "<?php echo site_url(); ?>app_accounting_report/estado_de_resultado/viewReport/true/periodID/"+periodID+"/cycleID/"+cycleID+"/classID/"+classID;
							}
							else{
						...
						$(document).on("change","#txtMayorizatePeriod",function(){
								...
								$.ajax({									
									...
									dataType    : 'json',
									type        : 'POST',
									url  		: "<?php echo site_url(); ?>app_accounting_api/getCycle",
