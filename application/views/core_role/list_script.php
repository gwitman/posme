	...
	$(document).ready(function(){		
		...
		$(document).on("click","#btnEdit",function(){
			...
			if(objRowTableListView != undefined){
				...
				window.location	= "<?php echo site_url(); ?>core_role/edit/companyID/"+data[0]+"/branchID/"+data[1]+"/roleID/"+data[2];
			}
			else{
			...
			if(objRowTableListView != undefined){
				...
				fnShowConfirm("Confirmar..","Desea eliminar este Rol...",function(){
					...
					$.ajax({									
						...
						dataType    : 'json',
						type        : 'POST',
						url  		: "<?php echo site_url(); ?>core_role/delete",
		...
		$(document).on("click","#btnNuevo",function(){
			fnWaitOpen();
			window.location	= "<?php echo site_url(); ?>core_role/add";
		});
	});
