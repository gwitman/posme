	...
	$(document).ready(function(){	
		...
		$(document).on("click","#btnEdit",function(){
			...
			if(objRowTableListView != undefined){
				...
				window.location	= "<?php echo site_url(); ?>app_invoice_billing/edit/companyID/"+data[0]+"/transactionID/"+data[1]+"/transactionMasterID/"+data[2];
			}
			else{
		...
		$(document).on("click","#btnSearchTransaction",function(){
					...
					$.ajax({									
						...
						dataType    : 'json',
						type        : 'POST',
						url  		: "<?php echo site_url(); ?>app_invoice_billing/searchTransactionMaster",
						...
						success:function(data){
							...
							if(data.error){
								...
								window.location = "<?php echo site_url(); ?>app_invoice_billing/edit/companyID/"+data.companyID+"/transactionID/"+data.transactionID+"/transactionMasterID/"+data.transactionMasterID;
							}
						},
		...
		$(document).on("click","#btnNuevo",function(){
			fnWaitOpen();
			window.location	= "<?php echo site_url(); ?>app_invoice_billing/add.aspx";
		});
	});
