...
class App_Accounting_Level extends CI_Controller {
    ...
    function edit(){ 
		 try{ 
			...
			$accountLevelID	= $uri["accountLevelID"];	
			$branchID 		= $dataSession["user"]->branchID;
			$roleID 		= $dataSession["role"]->roleID;			
			if((!$companyID || !$accountLevelID))
			{ 
				redirect('app_accounting_level/add');	
			} 		
			...
			$dataSession["head"]			= $this->load->view('app_account_level/edit_head',$datView,true);
			$dataSession["body"]			= $this->load->view('app_account_level/edit_body',$datView,true);
			$dataSession["script"]			= $this->load->view('app_account_level/edit_script',$datView,true);  
			$dataSession["footer"]			= "";				
	...
	function delete(){
		try{ 
			...
			$accountLevelID 	= $this->input->post("accountLevelID");				
			...
			$this->Account_Level_Model->delete($companyID,$accountLevelID);
	...
	function save($method = NULL){
		 try{ 
			...
			$this->form_validation->set_rules("txtLengthTotal","Longitud Total","callback_number_check");
			$this->form_validation->set_rules("txtLengthGroup","Longitud del Grupo","callback_number_check");
			...
			if( $method == "new" && $this->form_validation->run() == true ){
					...
					if($continue){
						...
						$obj["split"] 				= $this->input->post("txtSplit");
						$obj["description"] 		= $this->input->post("txtDescription");
						$obj["lengthTotal"] 		= $this->input->post("txtLengthTotal");	
						$obj["lengthGroup"] 		= $this->input->post("txtLengthGroup");					 
						$obj["isOperative"] 		= $this->input->post("txtIsOperative");
						$obj["isActive"] 			= true;
						$this->core_web_auditoria->setAuditCreated($obj,$dataSession);
						
						$accountLevelID				= $this->Account_Level_Model->insert($obj);
						$companyID 					= $obj["companyID"];
						
						if($this->db->trans_status() !== false){
							$this->db->trans_commit();						
							$this->core_web_notification->set_message(false,SUCCESS);
							redirect('app_accounting_level/edit/companyID/'.$companyID."/accountLevelID/".$accountLevelID);						
						}
						else{
			...
			//Editar Registro
					...
					$accountLevelID		= $this->input->post("txtAccountLevelID");
					...
					if($continue){
						...
						$obj["lengthTotal"] = $this->input->post("txtLengthTotal");
						$obj["isOperative"] = $this->input->post("txtIsOperative");						
						$result 			= $this->Account_Level_Model->update($companyID,$accountLevelID,$obj);
	...
	function add(){ 
		...
		try{ 
			...
			$dataSession["notification"]	= $this->core_web_error->get_error($dataSession["user"]->userID);
			$dataSession["message"]			= $this->core_web_notification->get_message();
			$dataSession["head"]			= $this->load->view('app_account_level/news_head','',true);
			$dataSession["body"]			= $this->load->view('app_account_level/news_body','',true);
			$dataSession["script"]			= $this->load->view('app_account_level/news_script','',true);  
			$dataSession["footer"]			= "";
			$this->load->view("core_masterpage/default_masterpage",$dataSession);	
	...
	try{ 
			...
			$dataSession["notification"]	= $this->core_web_error->get_error($dataSession["user"]->userID);
			$dataSession["message"]			= $this->core_web_notification->get_message();
			$dataSession["head"]			= $this->load->view('app_account_level/list_head','',true);
			$dataSession["footer"]			= $this->load->view('app_account_level/list_footer','',true);
			$dataSession["body"]			= $dataViewRender; 
			$dataSession["script"]			= $this->load->view('app_account_level/list_script','',true).$this->core_web_javascript->createVar("componentID",$objComponent->componentID);  
			$this->load->view("core_masterpage/default_masterpage",$dataSession);		
		}
		catch(Exception $ex){
			show_error($ex->getLine()." ".$ex->getMessage() ,500 );
