...
class App_Inventory_OtherInput extends CI_Controller {
	...
	function viewRegister(){
		try{ 
			...
			$reportName		= "DOC_OTRAS_ENTRADAS_A_INVENTARIO";
			//Set Informacion File
			$pdf->addInfo(array('Title'=>$reportName,'Author'=>APP_NAME,'CreationDate'=>date('Y-m-d H:i:s')));
			//Set Titulo			
			$pdf->EXTCreateHeader("::".$objCompany->name." OTRAS ENTRADAS A INVENTARIO::",$objComponent->componentID,$objParameter->value,$dataSession);
	...
	function insertElement($dataSession){
		try{
			...
			$objTM["targetWarehouseID"]				= $this->input->post("txtWarehouseSourceID",'');
			$objTM["isActive"]						= 1;
			...
			if($this->db->trans_status() !== false){
				...
				redirect('app_inventory_otherinput/edit/companyID/'.$companyID."/transactionID/".$objTM["transactionID"]."/transactionMasterID/".$transactionMasterID);
			}
			else{
				$this->db->trans_rollback();						
				$this->core_web_notification->set_message(true,$this->db->_error_message());
				redirect('app_inventory_otherinput/add');	
			}
	...
	function updateElement($dataSession){
		try{
			...
			$objTMNew["targetWarehouseID"]			= $this->input->post("txtWarehouseSourceID",'');						$objTMNew["amount"] 					= $this->input->post("txtTotal");
			$this->db->trans_begin();
			...
			if($this->db->trans_status() !== false){
				$this->db->trans_commit();						
				$this->core_web_notification->set_message(false,SUCCESS);
				redirect('app_inventory_otherinput/edit/companyID/'.$companyID."/transactionID/".$transactionID."/transactionMasterID/".$transactionMasterID);
			}
			else{
				$this->db->trans_rollback();						
				$this->core_web_notification->set_message(true,$this->db->_error_message());
	...
	function edit(){
		try{ 
			...
			$dataSession["head"]			= $this->load->view('app_inventory_otherinput/edit_head',$datView,true);
			$dataSession["body"]			= $this->load->view('app_inventory_otherinput/edit_body',$datView,true);
			$dataSession["script"]			= $this->load->view('app_inventory_otherinput/edit_script',$datView,true);  
			$dataSession["footer"]			= "";				
	...
	function add(){
		try{ 
			...
			$dataSession["notification"]		= $this->core_web_error->get_error($dataSession["user"]->userID);
			$dataSession["message"]				= $this->core_web_notification->get_message();
			$dataSession["head"]				= $this->load->view('app_inventory_otherinput/news_head',$dataView,true);
			$dataSession["body"]				= $this->load->view('app_inventory_otherinput/news_body',$dataView,true);
			$dataSession["script"]				= $this->load->view('app_inventory_otherinput/news_script',$dataView,true);  
			$dataSession["footer"]				= "";
			$this->load->view("core_masterpage/default_masterpage",$dataSession);	
	...
	try{
			...
			$dataSession["notification"]	= $this->core_web_error->get_error($dataSession["user"]->userID);
			$dataSession["message"]			= $this->core_web_notification->get_message();
			$dataSession["head"]			= $this->load->view('app_inventory_otherinput/list_head','',true);
			$dataSession["footer"]			= $this->load->view('app_inventory_otherinput/list_footer','',true);
			$dataSession["body"]			= $dataViewRender; 
			$dataSession["script"]			= $this->load->view('app_inventory_otherinput/list_script','',true).$this->core_web_javascript->createVar("componentID",$objComponent->componentID);  
			$this->load->view("core_masterpage/default_masterpage",$dataSession);		
		}
		catch(Exception $ex){
			show_error($ex->getLine()." ".$ex->getMessage() ,500 );
