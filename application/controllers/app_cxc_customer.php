	...
	function updateElement($dataSession){
		try{
			...
			if($this->db->trans_status() !== false){
				...
				redirect('app_cxc_customer/edit/companyID/'.$companyID_."/branchID/".$branchID_."/entityID/".$entityID_."/callback/".$callback);
			}
			else{
	...
	function insertElement($dataSession){
		try{
			...
			if($this->db->trans_status() !== false){
				$this->db->trans_commit();						
				$this->core_web_notification->set_message(false,SUCCESS);
				redirect('app_cxc_customer/edit/companyID/'.$companyID."/branchID/".$objEntity["branchID"]."/entityID/".$entityID."/callback/".$callback);
			}
			else{
				$this->db->trans_rollback();						
				$this->core_web_notification->set_message(true,$this->db->_error_message());
