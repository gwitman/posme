...
class App_Purchase_Returnsprovider extends CI_Controller {
	...
	function viewRegister(){
		try{ 
			...
			$reportName		= "DOC_DEVOL_COMPRA";
			//Set Informacion File
			$pdf->addInfo(array('Title'=>$reportName,'Author'=>APP_NAME,'CreationDate'=>date('Y-m-d H:i:s')));
			//Set Titulo			
			$pdf->EXTCreateHeader("::".$objCompany->name." DEVOLUCION DE COMPRA"."::",$objComponent->componentID,$objParameter->value,$dataSession);
			...
			$data = array( 
				...
				array('field1'=>'<b>Estado</b>'			,'field2'=>$datView["objTM"]->workflowStageName				,'field3'=>'<b>Referencia3</b>'		,'field4'=>$datView["objTM"]->reference3	) ,
				array('field1'=>'<b>Bodega</b>'			,'field2'=>$datView["objWarehouse"]->name					,'field3'=>''						,'field4'=>''	) 
			...
			foreach($datView["objTMD"] as $row){
				$razon 		= "";
				foreach($datView["objCatalogItemRazon"] as $ir ){
					if($ir->catalogItemID == $row->catalogStatusID)
						$razon = $ir->display;
				}
				...
				$data[] 	= array(
					...
					'field2' => $row->itemName,
					'field3' => $row->unitMeasureName,
					'field4' => $row->quantity,
					'field5' => $razon, 
					'field6' => $row->reference1 , 
					'field7' => $row->reference2
				);
			}
			
			$pdf->ezTable(
				$data,
				array('field1'=>'Codigo','field2'=>'Nombre','field3'=>'U/M','field4'=>'Cantidad','field5'=>'Razon','field6'=>'Lote', 'field7' => 'Vencimiento'),
				'',				
				array(
	...
	function insertElement($dataSession){
		try{
			...
			$arrayListTransactionDetail					= $this->input->post("txtDetailTransactionDetailID");
			...
			$arrayListTipo								= $this->input->post("txtDetailTipo");			
			...
			if($this->db->trans_status() !== false){
				...
				redirect('app_purchase_returnsprovider/edit/companyID/'.$companyID."/transactionID/".$objTM["transactionID"]."/transactionMasterID/".$transactionMasterID);
			}
			else{
				$this->db->trans_rollback();						
				$this->core_web_notification->set_message(true,$this->db->_error_message());
				redirect('app_purchase_returnsprovider/add');	
			}
	...
	function updateElement($dataSession){
		try{
			...
			if($this->db->trans_status() !== false){
				$this->db->trans_commit();						
				$this->core_web_notification->set_message(false,SUCCESS);
				redirect('app_purchase_returnsprovider/edit/companyID/'.$companyID."/transactionID/".$transactionID."/transactionMasterID/".$transactionMasterID);
			}
			else{
				$this->db->trans_rollback();						
				$this->core_web_notification->set_message(true,$this->db->_error_message());
	...
	function edit(){
		try{ 
			...
			$dataSession["head"]			= $this->load->view('app_purchase_returnsprovider/edit_head',$datView,true);
			$dataSession["body"]			= $this->load->view('app_purchase_returnsprovider/edit_body',$datView,true);
			$dataSession["script"]			= $this->load->view('app_purchase_returnsprovider/edit_script',$datView,true);  
			$dataSession["footer"]			= "";				
	...
	function add(){
		try{ 
			...
			$dataSession["notification"]		= $this->core_web_error->get_error($dataSession["user"]->userID);
			$dataSession["message"]				= $this->core_web_notification->get_message();
			$dataSession["head"]				= $this->load->view('app_purchase_returnsprovider/news_head',$dataView,true);
			$dataSession["body"]				= $this->load->view('app_purchase_returnsprovider/news_body',$dataView,true);
			$dataSession["script"]				= $this->load->view('app_purchase_returnsprovider/news_script',$dataView,true);  
			$dataSession["footer"]				= "";
			$this->load->view("core_masterpage/default_masterpage",$dataSession);	
	...
	try{
			...
			$dataSession["notification"]	= $this->core_web_error->get_error($dataSession["user"]->userID);
			$dataSession["message"]			= $this->core_web_notification->get_message();
			$dataSession["head"]			= $this->load->view('app_purchase_returnsprovider/list_head','',true);
			$dataSession["footer"]			= $this->load->view('app_purchase_returnsprovider/list_footer','',true);
			$dataSession["body"]			= $dataViewRender; 
			$dataSession["script"]			= $this->load->view('app_purchase_returnsprovider/list_script','',true).$this->core_web_javascript->createVar("componentID",$objComponent->componentID);  
			$this->load->view("core_masterpage/default_masterpage",$dataSession);		
		}
		catch(Exception $ex){
			show_error($ex->getLine()." ".$ex->getMessage() ,500 );
