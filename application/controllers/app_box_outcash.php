...
class App_Box_Outcash extends CI_Controller {
    ...
    function edit(){ 
		 try{ 
			...
			$dataSession["head"]			= $this->load->view('app_box_outcash/edit_head',$dataView,true);
			$dataSession["body"]			= $this->load->view('app_box_outcash/edit_body',$dataView,true);
			$dataSession["script"]			= $this->load->view('app_box_outcash/edit_script',$dataView,true);
			$dataSession["footer"]			= "";
		...
		try{
			...
			if(!empty($arrayListTransactionDetailID)){				
				...
				if($transactionDetailID == 0){	
					log_message("error",print_r("punto de interrupcion**************7.1",true));
					$objTMD 								= NULL;
				...
				//Editar Detalle
					...
					log_message("error",print_r("punto de interrupcion**************7.2",true));
					...
					log_message("error",print_r("punto de interrupcion**************7.3",true));
			...
			if($this->db->trans_status() !== false){
				...
				redirect('app_box_outcash/edit/companyID/'.$companyID."/transactionID/".$transactionID."/transactionMasterID/".$transactionMasterID);
	...
	function insertElement($dataSession){
		try{
			...
			if($this->db->trans_status() !== false){
				$this->db->trans_commit();						
				$this->core_web_notification->set_message(false,SUCCESS);
				redirect('app_box_outcash/edit/companyID/'.$companyID."/transactionID/".$objTM["transactionID"]."/transactionMasterID/".$transactionMasterID);
			}
			else{
				$this->db->trans_rollback();						
				$this->core_web_notification->set_message(true,$this->db->_error_message());
	...
	function add(){ 
		...
		try{ 
			...
			$dataSession["notification"]	= $this->core_web_error->get_error($dataSession["user"]->userID);
			$dataSession["message"]			= $this->core_web_notification->get_message();
			$dataSession["head"]			= $this->load->view('app_box_outcash/news_head',$dataView,true);
			$dataSession["body"]			= $this->load->view('app_box_outcash/news_body',$dataView,true);
			$dataSession["script"]			= $this->load->view('app_box_outcash/news_script',$dataView,true);  
			$dataSession["footer"]			= "";
			$this->load->view("core_masterpage/default_masterpage",$dataSession);	
	...
	function index($dataViewID = null){	
		try{ 
			...
			//Vista por defecto MOBILE
				...
				$parameter["{companyID}"]	= $this->session->userdata('user')->companyID;
				$dataViewData				= $this->core_web_view->getViewByName($this->session->userdata('user'),$objComponent->componentID,"DEFAULT_MOBILE_LISTA_EGRESOS_A_CAJA",CALLERID_LIST,$resultPermission,$parameter); 			
				$dataViewRender				= $this->core_web_view->renderGreed($dataViewData,'ListView',"fnTableSelectedRow");
			} 
			//Vista Por Id
			else 
			...
			$dataSession["notification"]	= $this->core_web_error->get_error($dataSession["user"]->userID);
			$dataSession["message"]			= $this->core_web_notification->get_message();
			$dataSession["head"]			= $this->load->view('app_box_outcash/list_head','',true);
			$dataSession["footer"]			= $this->load->view('app_box_outcash/list_footer','',true);
			$dataSession["body"]			= $dataViewRender; 
			$dataSession["script"]			= $this->load->view('app_box_outcash/list_script','',true).$this->core_web_javascript->createVar("componentID",$objComponent->componentID);  
			$this->load->view("core_masterpage/default_masterpage",$dataSession);		
		}
		catch(Exception $ex){
			show_error($ex->getLine()." ".$ex->getMessage() ,500 );
