...
class App_Inventory_RequestGeneral extends CI_Controller {
	...
	function viewRegister(){
		try{ 
			...
			$reportName		= "DOC_SOLICITUD_GENERAL";
			//Set Informacion File
			$pdf->addInfo(array('Title'=>$reportName,'Author'=>APP_NAME,'CreationDate'=>date('Y-m-d H:i:s')));
			//Set Titulo			
			$pdf->EXTCreateHeader("::".$objCompany->name." SOLICITUD GENERAL"."::",$objComponent->componentID,$objParameter->value,$dataSession);
			...
			$data = array( 
				...
				array('field1'=>'<b>Usuario</b>'		,'field2'=>$datView["objEmployee"] == NULL ? $datView["objUser"]->nickname : $datView["objEmployee"]->employeNumber ." " .$datView["objNatural"]->firstName ." ".$datView["objNatural"]->lastName					
																													,'field3'=>''					,'field4'=>''									) 				
			);		
			$pdf->ezTable(
				$data,
	...
	function insertElement($dataSession){
		try{
			...
			if($this->db->trans_status() !== false){
				...
				redirect('app_inventory_requestgeneral/edit/companyID/'.$companyID."/transactionID/".$objTM["transactionID"]."/transactionMasterID/".$transactionMasterID);
			}
			else{
				$this->db->trans_rollback();						
				$this->core_web_notification->set_message(true,$this->db->_error_message());
				redirect('app_inventory_requestgeneral/add');	
			}
	...
	function updateElement($dataSession){
		try{
			...
			if($this->db->trans_status() !== false){
				$this->db->trans_commit();						
				$this->core_web_notification->set_message(false,SUCCESS);
				redirect('app_inventory_requestgeneral/edit/companyID/'.$companyID."/transactionID/".$transactionID."/transactionMasterID/".$transactionMasterID);
			}
			else{
				$this->db->trans_rollback();						
				$this->core_web_notification->set_message(true,$this->db->_error_message());
	...
	function edit(){
		try{ 
			...
			$dataSession["head"]			= $this->load->view('app_inventory_requestgeneral/edit_head',$datView,true);
			$dataSession["body"]			= $this->load->view('app_inventory_requestgeneral/edit_body',$datView,true);
			$dataSession["script"]			= $this->load->view('app_inventory_requestgeneral/edit_script',$datView,true);  
			$dataSession["footer"]			= "";				
	...
	function add(){
		try{ 
			...
			$dataSession["notification"]		= $this->core_web_error->get_error($dataSession["user"]->userID);
			$dataSession["message"]				= $this->core_web_notification->get_message();
			$dataSession["head"]				= $this->load->view('app_inventory_requestgeneral/news_head',$dataView,true);
			$dataSession["body"]				= $this->load->view('app_inventory_requestgeneral/news_body',$dataView,true);
			$dataSession["script"]				= $this->load->view('app_inventory_requestgeneral/news_script',$dataView,true);  
			$dataSession["footer"]				= "";
			$this->load->view("core_masterpage/default_masterpage",$dataSession);	
	...
	try{
			...
			$dataSession["notification"]	= $this->core_web_error->get_error($dataSession["user"]->userID);
			$dataSession["message"]			= $this->core_web_notification->get_message();
			$dataSession["head"]			= $this->load->view('app_inventory_requestgeneral/list_head','',true);
			$dataSession["footer"]			= $this->load->view('app_inventory_requestgeneral/list_footer','',true);
			$dataSession["body"]			= $dataViewRender; 
			$dataSession["script"]			= $this->load->view('app_inventory_requestgeneral/list_script','',true).$this->core_web_javascript->createVar("componentID",$objComponent->componentID);  
			$this->load->view("core_masterpage/default_masterpage",$dataSession);		
		}
		catch(Exception $ex){
			show_error($ex->getLine()." ".$ex->getMessage() ,500 );
